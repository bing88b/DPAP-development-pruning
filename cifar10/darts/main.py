...
# Dataset parameters ONLY FOR IMAGE NET
...
parser.add_argument('--output', default='/data/floyed/SpikingNN/darts', type=str, metavar='PATH',
...
#DARTS parameters
parser.add_argument('--init-channels', type=int, default=64)
...
def main():
    ...
    if args.start_epoch is not None:
        ...
        start_epoch = args.start_epochzhou
    ...
    if args.dataset == 'imnet':
        ...
        loader_train, loader_eval, mixup_active, mixup_fn = get_imnet_data(args, _logger, data_config, num_aug_splits)
    else:
        loader_train, loader_eval = eval('get_%s_data' % args.dataset)(batch_size=args.batch_size)
        mixup_active = False
        mixup_fn = None

    if args.loss_fn == 'mix':
        train_loss_fn = MixLoss(LabelSmoothingCrossEntropy(smoothing=args.smoothing).cuda())
        validate_loss_fn = MixLoss(nn.CrossEntropyLoss())

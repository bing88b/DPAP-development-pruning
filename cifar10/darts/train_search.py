...
from SpikingNN.datasets.datasets import build_transform
...
parser.add_argument('--node_type', default='PLIFNode', type=str)
parser.add_argument('--loss_fn', type=str, default='mix')
parser.add_argument('--suffix', default='', type=str)
args = parser.parse_args()

args.save = '/data/floyed/darts/logs/search/search-{}-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"), args.suffix)
...
def main():
    ...
    if args.loss_fn == 'mix':
        criterion_train = MixLoss(LabelSmoothingCrossEntropy(smoothing=args.smoothing).cuda())
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_acc, train_obj = train(epoch, train_queue, valid_queue, model, architect, criterion_train, model_optimizer)
...
def train(epoch, train_queue, valid_queue, model, architect, criterion, model_optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if epoch >= 10:
            ...
            loss_arch = architect.step(input_search, target_search)
        else:
            loss_arch = torch.tensor([0.])
        ...
        if step % args.report_freq == 0:
            logging.info('train %03d loss: %e loss_arch: %e top1: %f top5: %f',
                         step, objs.avg, objs_arch.avg, top1.avg, top5.avg)

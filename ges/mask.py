...
class Mask:
    ...
    def get_filter_codebook(self, weight_torch, pruning, length, wfc1, wconv2,wconv11=0,wconv22=0, wwfc1=0, index=0,epoch=0):
        if len(weight_torch.size()) == 4:
            ...
            if index==2:
                ...
                delta_wwcon=(unit(filter_wwcon)*2-0.7)
                ...
                # self.n_delta[index]=(unit(wconv)*2-0.7)
        ...
        if len(weight_torch.size()) == 2:
            if index == 4:
                ...
                delta_ww=(unit(filter_ww)*2-0.67)
                pos=np.where(delta_ww>0)[0]
                delta_ww[pos]=delta_ww[pos]+2
                self.reduceww= self.reduceww*0.999+delta_ww*math.exp(-int((epoch-0)/10))
                dq.append(self.reduceww)
                filter_indww = np.where(self.reduceww < 0)[0]
                self.fullbook[index][filter_indww] = 0
                print(self.reduceww.mean(),self.reduceww.max(),self.reduceww.min())
                ...
                # self.n_delta[index]=(unit(wfc1)*2-1)
                # pos=np.where(self.n_delta[index]>0)[0]
                # self.n_delta[index][pos]=self.n_delta[index][pos]+2
                # self.reduce[index]=self.reduce[index]*0.999+self.n_delta[index]*math.exp(-int((epoch-0)/9))
                # dqn.append(self.reduce[index])
                # filter_ind = np.where(self.reduce[index] < 0)[0]
                # print(self.reduce[index].mean(),self.reduce[index].max(),self.reduce[index].min(),pruning)
    ...
    def init_length(self):
        ...
        for index1 in self.model_size:
            for index2 in range(0, len(self.model_size[index1])):
                if index2 == 0:
                    ...
                    self.reduce[index1] = 10*np.ones(self.model_size[index1][0])
        ...
        self.reduceww=10*np.ones(self.model_length[4])
        self.reducewwcon=10*np.ones(int(self.model_length[2]/9))
        self.his_ww=[]
        self.his_nn=[]

...
def mem_update(ops, x, mem, spike):
    #print(mem.size(),spike.size(), ops(x).size())
...
class SCNN(nn.Module):
    ...
    def stdp(self,pres,posts,stdpwin=8,time_window=32):
    ...
    def forward(self, input, mat,time_window=32, epoch=0, train=0):
        ...
        for step in range(time_window):  # simulation time steps
            ...
            c1_mem, c1_spike = mem_update(self.conv1, cx.float(), c1_mem, c1_spike)
...
#     def forward(self, input, time_window=20, epoch=0, train=0):
#         c1_mem = c1_spike = torch.zeros(batch_size, cfg_cnn[0][1], cfg_kernel[0], cfg_kernel[0], device=device)
#         c2_mem = c2_spike = torch.zeros(batch_size, cfg_cnn[1][1], cfg_kernel[1], cfg_kernel[1], device=device)
...
#             if (train != 0):
#                 c1_strace=delta*c1_strace+c1_spike
#                 c2_strace=delta*c2_strace+c2_strace
#                 xsum=delta*xsum+x
#                 h1sum=delta*h1sum+ h1_spike

#         outputs = h2_sumspike / time_window
#         if (train != 0):
#             c1_strace = torch.sum(c1_strace, dim=0)
#             c1sum=c1_strace/(time_window*batch_size)

#             c2_strace = torch.sum(c2_strace, dim=0)
#             c2sum=c2_strace/(time_window*batch_size)

#             #xmean = torch.sum(xsum,dim=0)
#             xmean=xsum/(time_window)

#             #h1mean = torch.sum(h1sum, dim=0)
#             h1mean=h1sum/(time_window)
...
#             return outputs,xmean,h1mean,c1sum,c2sum
#         else:
#             return outputs

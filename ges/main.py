...
statg=torch.load('/home/hanbing/project/nng/checkpoint/ckptwwg.t7')
bcmwg=torch.tensor(statg['bcmw'])
bcmng=torch.tensor(statg['bcmn'])
print(bcmwg.size())
print(bcmng.size())
meawg,stwg=torch.mean(bcmwg,axis=0),torch.std(bcmwg,axis=0)
meanng,stng=torch.mean(bcmng,axis=0),torch.std(bcmng,axis=0)
print(meawg)
print(stwg)
print(meanng)
print(stng)
...
print(len(train_datasets),len(test_loader))
...
exp_name = '-'.join([datetime.now().strftime("%Y%m%d-%H%M%S"),'dvsg','conv-0.7-10-fc-0.775-9'])
